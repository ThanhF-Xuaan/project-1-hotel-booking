...
| **Online Room Selection** | Physical Room Slot (`room_slots`) | **Single Pessimistic Lock (`SELECT FOR UPDATE`)** | Execute a database row-level write lock on the specific stay dates. *Ensures exclusive access to a unique physical room (e.g., Room 101), preventing two checked-out guests from selecting the same physical room.* |
