...
CROSS JOIN (
    VALUES 
        ...
        ('ADULT', 12, 99)   -- Người lớn: Từ 12 tuổi trở lên (đặt max là 99)
...
-- 3. Chiến dịch Đón tuyết cho Viettel Boutique Sapa (Setup trước cho cuối năm)
INSERT INTO campaigns (hotel_id, name, description, start_date, end_date, status)
SELECT 
...
-- Bảng: catalog_items 
-- (Bán tại chỗ theo số lượng thực tế khách dùng)
-- ==============================================================================

INSERT INTO catalog_items (hotel_id, name, item_type, tax_category_id, base_price, status)
SELECT 
    h.id,
    c.name,
    c.item_type,
    (SELECT id FROM tax_categories WHERE category_code = c.tax_code),
    c.base_price,
    'ACTIVE'
FROM hotels h
CROSS JOIN (
...
) AS c(name, item_type, tax_code, base_price);
...
INSERT INTO hotel_room_types (
    ...
    max_beds, max_extra_beds, 
    base_price, total_quantity, status
)
VALUES 
    -- ========================================================================
    -- 1. VIETTEL LUXURY HÀ NỘI (HẠNG 5 SAO) - ID Khách sạn động
    -- ========================================================================
    (
        (SELECT id FROM hotels WHERE name = 'Viettel Luxury Hà Nội'), (SELECT id FROM room_types WHERE code = 'DLX'),
        2, 0, 2, 1, 1, 3, 1, 1, 2500000.00, 20, 'ACTIVE'
    ),
    (
        (SELECT id FROM hotels WHERE name = 'Viettel Luxury Hà Nội'), (SELECT id FROM room_types WHERE code = 'STE'),
        2, 0, 3, 2, 1, 4, 1, 1, 4500000.00, 10, 'ACTIVE'
    ),
    (
        (SELECT id FROM hotels WHERE name = 'Viettel Luxury Hà Nội'), (SELECT id FROM room_types WHERE code = 'EXE'),
        2, 0, 2, 1, 1, 3, 1, 0, 10000000.00, 2, 'ACTIVE' -- Phòng Tổng thống không cho kê thêm giường
    ),
    ...
    -- ========================================================================
    (
        (SELECT id FROM hotels WHERE name = 'Viettel Grand Đà Nẵng'), (SELECT id FROM room_types WHERE code = 'SUP'),
        2, 0, 2, 1, 1, 3, 1, 1, 1200000.00, 30, 'ACTIVE'
    ),
    (
        (SELECT id FROM hotels WHERE name = 'Viettel Grand Đà Nẵng'), (SELECT id FROM room_types WHERE code = 'DLX'),
        2, 0, 3, 2, 1, 4, 2, 0, 1800000.00, 15, 'ACTIVE'
    ),
    (
        (SELECT id FROM hotels WHERE name = 'Viettel Grand Đà Nẵng'), (SELECT id FROM room_types WHERE code = 'FAM'),
        4, 0, 4, 2, 2, 6, 2, 1, 3000000.00, 5, 'ACTIVE' -- Family cho 4 người lớn chuẩn
    ),
    ...
    -- ========================================================================
    (
        (SELECT id FROM hotels WHERE name = 'Viettel Boutique Sapa'), (SELECT id FROM room_types WHERE code = 'STD'),
        2, 0, 2, 1, 1, 2, 1, 0, 600000.00, 20, 'ACTIVE' -- Standard bé, tối đa 2 người, không extra bed
    ),
    (
        (SELECT id FROM hotels WHERE name = 'Viettel Boutique Sapa'), (SELECT id FROM room_types WHERE code = 'SUP'),
